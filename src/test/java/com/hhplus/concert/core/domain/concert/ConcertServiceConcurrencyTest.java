...
public class ConcertServiceConcurrencyTest extends IntegrationTest {
    ...
    @Test
    void 좌석_임시예약_낙관적락_동시성_테스트() throws InterruptedException {
        // given
        int numberOfThreads = 100;
        ExecutorService service = Executors.newFixedThreadPool(numberOfThreads);
        CountDownLatch latch = new CountDownLatch(numberOfThreads);
    ...
    void 동일_유저가_여러번_결제요청을_해도_정상적으로_처리된다_낙관적락() throws InterruptedException {
        ...
        // 예약 생성 및 저장
        ReserveConcertResult result = concertService.reserveConcertWithOptimisticLock(token, concertSchedule.getId(), concertSeat.getId());
